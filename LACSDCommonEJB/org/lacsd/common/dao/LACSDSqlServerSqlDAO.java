...
//* Filename:		LACSDSqlServerSqlDAO.java
//* Revision: 		1.0
//* Author:			M Feinberg
...
public abstract class LACSDSqlServerSqlDAO extends LACSDSqlServerDAO {
 ...
 * Constructor - 
*/
public LACSDSqlServerSqlDAO() {
	super();
	this.ownerID = "";
}

public LACSDSqlServerSqlDAO(String otherDatabaseName) {
...
*/
public LACSDSqlServerSqlDAO(boolean useLogging) {
	this();
	...
	//	conn.setAutoCommit(false);	//	Added 08-31-04 (MF) - ConnectionSimplePolicy.java commits!  MySQL jives with this!
		...
		switch (queryType) {
			...
			case EXECUTE_QUERY:	rs = statement.executeQuery(sqlStatement);
									...
									tm.setTimeMark(2);
									if (logging) {
										log.debug("SqlSrvr Query returned [" + rows + "] rows with [" + cols +"] columns in: [" + tm.compareTimeMilli(1,2) + "] milliseconds");
									}
			...
			case EXECUTE_UPDATE:	statement.executeUpdate(sqlStatement);
									tm.setTimeMark(2);
									if (logging) {
										log.debug("SqlSrvr Updated successfully in: [" + tm.compareTimeMilli(1,2) + "] milliseconds");
									}
									break;
		...
		throw new LACSDSqlSrvr2KException("Unable to execute sql statement", sqle.fillInStackTrace());
	}
	catch (Throwable t) {
		...
		if (t instanceof LACSDException) {
			...
			throw new LACSDSqlSrvr2KException("Unable to execute sql statement", t.fillInStackTrace());
		}
	}
	finally {
