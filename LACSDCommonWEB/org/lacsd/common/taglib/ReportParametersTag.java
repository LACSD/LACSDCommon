...
public class ReportParametersTag extends TagSupport {
   /**
	 * 
	 */
   private static final long serialVersionUID = -4013558817465388068L;
   ...
   public void setWidth(String width) {
	...
	private String radioBoxFormat(ReportParameterVO parameterVO, int paramIndex, String required) {
       ...
       for(int i=0; i < paramVOs.length; i++) {
           ...
           strFormat += "<td><input tabindex=\"" + (paramIndex+1) + "\" type=\"radio\" name=\"radio" +paramIndex+ "\"  value=\"" + paramVOs[i].getValue().trim() + "\"   onclick=\"if (this.checked) document.getElementById('reportParameterVO[" + paramIndex + "].value').value=this.value;\"" ;
	...
	private String dropdownBoxFormat(ReportParameterVO parameterVO, int paramIndex, String required) {
       ...
       if (parameterVO.getControlType().equalsIgnoreCase(ReportParameterVO.MULTIPLE_DROPDOWN)) {
       	strFormat = "<input type=\"hidden\" name=\"reportParameterVO[" + paramIndex + "].value\" id=\"adHocReportParamValue" + paramIndex + "\"/>";
       	...
       	strFormat = "<SELECT   tabindex=\"" + (paramIndex+1) + "\" id=\""+ parameterVO.getName().trim() +"\" name=\"reportParameterVO[" + paramIndex + "].value\" " + required;
           strFormat += " size=\"1\">";
       }
	...
	private String textBoxFormat (ReportParameterVO parameterVO, int paramIndex, String required) {
	    ...
	    if (parameterVO.getName().equalsIgnoreCase(LACSDReportConstants.PARAM_TYPE_DUNNING_MSG)) {
	        ...
	        strFormat += "<input type=\"hidden\" name=\"reportParameterVO[" + paramIndex + "].value\" id=\"dunningMessageParams\" value=\"\">";
	    }else {
	        if (parameterVO.getType().equalsIgnoreCase(ReportParameterVO.DATE_ONLY_TYPE) ||
		        ...
		        strFormat = "<input tabindex=\"" + (paramIndex+1) + "\" type=\"" + (parameterVO.isHidden()?"hidden":"text") +"\" size=\"35\" id=\"" + parameterVO.getName() + "\" name=\"reportParameterVO[" + paramIndex + "].value\" " + required;
